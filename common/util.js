...
export default {
  ...
  undoListener(config) {
    ...
    this.redoListenerFunc2 = this.redoListenerFunc.bind(this, undoManager, callback);

    document.body.addEventListener('keydown', this.undoListenerFunc2);
    document.body.addEventListener('keydown', this.redoListenerFunc2);
  },
  ...
  redoListenerFunc(undoManager, callback, e) {
    // if (e.target !== e.currentTarget) {
    //   return false;
    // }
  ...
  saveXml(config) {
    ...
    window.localStorage.setItem('autosaveXml', xmlStr);
  ...
  renderGraphFromXml(config) {
    ...
    if (xml === null || xml === '') {
      ...
      xml = `<mxGraphModel grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="0" fold="1" page="0" pageScale="1.5" pageWidth="${browserWidth}" pageHeight="${browserHeight}"><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel>`;
    }
  ...
  initCustomPort(config) {
    const { pic, isEnabled } = config;
    // Replaces the port image
    // eslint-disable-next-line no-undef
    mxConstraintHandler.prototype.setEnabled(isEnabled); 
  ...
  setBackgroundImage(graph, dataURL, browserWidth, browserHeight) {
    // eslint-disable-next-line 
    graph.redrawBackgroundImage(new mxImage(dataURL, browserWidth, browserHeight));
  }
